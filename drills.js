...
function getMiddleNode(linkedList) {
  ...
  while (double.next !== null) {
    middle = node.next;
    countBy1++;
    if (double.next) double = double.next.next;
  }
...
//Can redo this using the getMiddleNode function and creating nodes for the loops instead of making a new LinkedList
...
// console.log(sortInPlace([1,2,42,6,34,9]))

function sortBook(arr) {
  arr = arr.map(bookName => bookName.toLowerCase())
  quickSort(arr)
  return arr;
}

const books = [
  'ouse of Representatives Speake', 'ad', 'lsdkjfsd', 'lksdjflsd', 'helolowehf', 'asdlkfj', 'Ouse of Representatives Speake'
]
